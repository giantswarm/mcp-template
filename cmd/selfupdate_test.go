...
func TestSelfUpdateCmdProperties(t *testing.T) {
	...
	if cmd.Short != "Update mcp-template to the latest version" {
		t.Errorf("Short = %q", cmd.Short)
	}
	for _, want := range []string{"mcp-template", "GitHub", "Sigstore bundle"} {
		if !strings.Contains(cmd.Long, want) {
			t.Errorf("the help should mention %q (releases are verified), got:\n%s", want, cmd.Long)
		}
	}
	// The slug is <owner>/<repo> of the module path, which scripts/init.sh
	// rewrites together with the module; a bare name or a URL would make
	// DetectLatest look in the wrong place.
	if githubRepoSlug != "giantswarm/mcp-template" {
		t.Errorf("githubRepoSlug = %q", githubRepoSlug)
	}
	if strings.Count(githubRepoSlug, "/") != 1 || strings.Contains(githubRepoSlug, "github.com") {
		t.Errorf("githubRepoSlug must be owner/repo, got %q", githubRepoSlug)
	}
}
...
// mcp-template wires it in so that an unsigned or unverifiable release never
// reaches the disk.
...
func (r fakeRelease) GetURL() string {
	return "https://github.com/" + githubRepoSlug + "/releases/tag/" + r.tag
}
...
func binaryAsset() string {
	name := serviceName + "-" + runtime.GOOS + "-" + runtime.GOARCH
...
func selfUpdateFixture(t *testing.T, src *fakeSource) (string, []byte) {
	t.Helper()
	installed := []byte("the " + serviceName + " that is installed right now")
	exe := filepath.Join(t.TempDir(), serviceName)
...
func TestRunSelfUpdateRefusesAReleaseWithoutASignatureBundle(t *testing.T) {
	src := &fakeSource{
		...
		assets:  map[int64][]byte{1: []byte("a newer binary, unsigned")},
	}
...
func TestRunSelfUpdateInstallsAVerifiedReleaseInPlace(t *testing.T) {
	...
	src := &fakeSource{
		...
		assets: map[int64][]byte{1: []byte("a newer mcp-template"), 2: []byte("its bundle")},
	}
	...
	got, err := os.ReadFile(exe) //nolint:gosec // the test's own temp file
	if err != nil || string(got) != "a newer mcp-template" {
