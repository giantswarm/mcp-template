...
func TestVersionCmd(t *testing.T) {
	tests := []struct {
		...
		{name: "dev version", version: "dev", firstLine: "mcp-template version dev"},
		{name: "semantic version", version: "v1.2.3", firstLine: "mcp-template version v1.2.3"},
		{name: "empty version", version: "", firstLine: "mcp-template version "},
	}
...
func TestVersionCmdProperties(t *testing.T) {
	...
	if cmd.Short != "Print the version number of mcp-template" {
		t.Errorf("Short = %q", cmd.Short)
	}
	if !strings.Contains(cmd.Long, "mcp-template") {
