...
// mcp-template binaries, one per platform, each next to its cosign Sigstore
// bundle. scripts/init.sh derives it from --module; the release assets exist
// only when the repo carries the `cli` flavour in giantswarm/github (see the
// README).
const githubRepoSlug = "giantswarm/mcp-template"
...
func newSelfUpdateCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "self-update",
		Short: "Update mcp-template to the latest version",
		Long: `Checks for the latest release of mcp-template on GitHub and
